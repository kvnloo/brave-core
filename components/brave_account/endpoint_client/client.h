...
using ReturnType = std::conditional_t<C == Cancelability::kCancelable,
...
class Client {
 ...
 private:
  ...
  static ReturnType<C> SendImpl(
    ...
    auto simple_url_loader_ptr = MakeSimpleURLLoaderPtr<C>(simple_url_loader);
    simple_url_loader_raw->DownloadToString(
        url_loader_factory.get(),
        base::BindOnce(std::move(on_response), std::move(callback),
                       std::move(simple_url_loader_ptr)),
